package com.aether.client.model;

public class AetherArmorModels {

    public static void registerArmorModels() {
        // TODO: [1.17] Fabric API does not contain this for the time being, uncomment when it's back or rewrite this.
//        ArmorRenderingRegistry.registerModel((entity, stack, slot, defaultModel) -> new PhoenixArmorModel(1), AetherItems.PHOENIX_HELMET);
//        ArmorRenderingRegistry.registerTexture((entity, stack, slot, secondLayer, suffix, defaultTexture) -> new Identifier("minecraft", "textures/models/armor/aether_phoenix_layer_1.png"), AetherItems.PHOENIX_HELMET);
    }
}
