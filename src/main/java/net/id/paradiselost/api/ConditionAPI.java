...
public class ConditionAPI {
    ...
    public static List<Condition> getValidConditions(EntityType<?> type) {
        return ParadiseLostRegistries.CONDITION_REGISTRY
                .stream()
    ...
    public static Condition getOrThrow(Identifier id) {
        return ParadiseLostRegistries.CONDITION_REGISTRY.getOrEmpty(id).orElseThrow((() -> new NoSuchElementException("No ConditionManager found registered for entry: " + id.toString())));
    }
    ...
    public static boolean isVisible(Condition condition, LivingEntity entity) {
        if(!condition.isExempt(entity)) {
            return ParadiseLostComponents.CONDITION_MANAGER_KEY.get(entity).getScaledSeverity(condition) >= condition.visThreshold;
        }
        return false;
    }
    ...
    public static ConditionManager getConditionManager(LivingEntity entity) {
        return ParadiseLostComponents.CONDITION_MANAGER_KEY.get(entity);
    }
    ...
    public static void trySync(LivingEntity entity) {
        ParadiseLostComponents.CONDITION_MANAGER_KEY.sync(entity);
    }
