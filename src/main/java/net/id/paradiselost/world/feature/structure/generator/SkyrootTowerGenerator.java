...
public class SkyrootTowerGenerator {
    private static final Identifier SKYROOT_TOWER = ParadiseLost.locate("skyroot_tower");
    ...
    public static class Piece extends SimpleStructurePiece {
        ...
        public Piece(StructureTemplateManager manager, Identifier template, BlockPos pos, BlockRotation rotation) {
            super(ParadiseLostStructureFeatures.SKYROOT_TOWER_PIECE, 0, manager, template, template.toString(), createPlacementData(rotation), pos);
        }

        public Piece(StructureTemplateManager manager, NbtCompound nbt) {
            super(ParadiseLostStructureFeatures.SKYROOT_TOWER_PIECE, nbt, manager, (identifier) -> createPlacementData(BlockRotation.valueOf(nbt.getString("Rot"))));
        }
        ...
        public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox boundingBox, ChunkPos chunkPos, BlockPos pos) {
            ...
            fillSupport(world, pos.down().north(-2).east(2), ParadiseLostBlocks.SKYROOT_LOG.getDefaultState());
            fillSupport(world, pos.down().north(-2).east(-2), ParadiseLostBlocks.SKYROOT_LOG.getDefaultState());
            fillSupport(world, pos.down(), ParadiseLostBlocks.STRIPPED_SKYROOT_LOG.getDefaultState());
            // Add path blocks
            for (int x = -2; x <= 2; x++) {
                for (int z = -2; z <= 2; z++) {
