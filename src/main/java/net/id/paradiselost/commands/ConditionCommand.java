...
public class ConditionCommand {
    ...
    private static int clearCondition(ServerCommandSource source, Collection<? extends Entity> entities, Identifier attributeId) {
        ...
        entities.forEach(entity -> {
            if(entity instanceof LivingEntity target) {
                ...
                if (!conditions.isEmpty()) {
                    ...
                    source.sendFeedback(
                            Text.translatable(
                                    "commands.paradise_lost.condition.success.clear.individual",
        ...
        source.sendFeedback(Text.translatable("commands.paradise_lost.condition.success.clear"), true);
        return 1;
    }
            ...
            if(entity instanceof LivingEntity target) {
                ...
                conditions.forEach(condition -> {
                    ...
                    if (!condition.isExempt(target)) {
                        ...
                        source.sendFeedback(Text.translatable("commands.paradise_lost.condition.success.query", Text.translatable(ConditionAPI.getTranslationString(condition)), Text.translatable(severity.getTranslationKey()), rawSeverity), false);
                    } else {
                        source.sendError(Text.translatable("commands.paradise_lost.condition.failure.query", Text.translatable(ConditionAPI.getTranslationString(condition))));
                    }
                });
            }
        });
    ...
    private static int setCondition(ServerCommandSource source, Entity entity, Identifier attributeId, float value, String persistenceString) {
        if(entity instanceof LivingEntity target) {
            ...
            try {
                ...
                source.sendError(Text.translatable("commands.paradise_lost.condition.failure.get_condition", attributeId));
                return 1;
            }
            try {
                persistence = Persistence.valueOf(persistenceString);
            } catch (NoSuchElementException e) {
                source.sendError(Text.translatable("commands.paradise_lost.condition.failure.get_persistence", persistenceString));
                return 1;
            }
            ...
            if(!condition.isExempt(target)) {
                if(manager.set(condition, persistence, value)) {
                    ...
                    source.sendFeedback(Text.translatable("commands.paradise_lost.condition.success.assign", Text.translatable(ConditionAPI.getTranslationString(condition)), Text.translatable(severity.getTranslationKey()), rawSeverity), false);
                    ConditionAPI.trySync(target);
                }
                else {
                    source.sendError(Text.translatable("commands.paradise_lost.condition.failure.assign"));
                }
            }
        }
        return 1;
    ...
    private static Collection<? extends Entity> handleNullEntity(ServerCommandSource source, Collection<? extends Entity> entities){
        if (entities == null) {
            try {
                ...
                source.sendError(Text.translatable("commands.paradise_lost.condition.failure.get_entity"));
                entities = List.of();
            }
        }
        return entities;
    ...
    public static class ConditionSuggester implements SuggestionProvider<ServerCommandSource> {
        ...
        public CompletableFuture<Suggestions> getSuggestions(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) throws CommandSyntaxException {
            ParadiseLostRegistries.CONDITION_REGISTRY.getIds().forEach(id -> builder.suggest(id.toString()));
