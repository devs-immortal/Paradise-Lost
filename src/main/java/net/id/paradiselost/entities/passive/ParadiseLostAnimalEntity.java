...
public class ParadiseLostAnimalEntity extends AnimalEntity {
    ...
    public static boolean isValidNaturalParadiseLostSpawn(EntityType<? extends ParadiseLostAnimalEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return (world.getBlockState(pos.down()).isOf(ParadiseLostBlocks.HIGHLANDS_GRASS) ||  world.getBlockState(pos.down()).isOf(ParadiseLostBlocks.FROZEN_GRASS)) && world.getBaseLightLevel(pos, 0) > 8;
    }

    @Override
    public float getPathfindingFavor(BlockPos pos, WorldView worldIn) {
        return (worldIn.getBlockState(pos.down()).isOf(ParadiseLostBlocks.HIGHLANDS_GRASS) ||  worldIn.getBlockState(pos.down()).isOf(ParadiseLostBlocks.FROZEN_GRASS)) ? 10.0F : worldIn.getLightLevel(pos) - 0.5F;
    }
