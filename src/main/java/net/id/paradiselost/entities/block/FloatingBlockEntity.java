...
public class FloatingBlockEntity extends Entity implements PostTickEntity {
    ...
    private BlockState floatTile = ParadiseLostBlocks.GRAVITITE_ORE.getDefaultState();
    ...
    public FloatingBlockEntity(World world, double x, double y, double z, BlockState floatingBlockState) {
        this(ParadiseLostEntityTypes.FLOATING_BLOCK, world);
    ...
    @Override
    public void tick() {
        if (this.floatTile.isIn(ParadiseLostBlockTags.HURTABLE_FLOATERS)) {
    ...
    protected void readCustomDataFromNbt(NbtCompound compound) {
        ...
        if (compound.contains("Dropping", 99)) {
            this.setDropping(compound.getBoolean("Dropping"));
        }
        ...
        if (this.floatTile.isAir()) {
            this.floatTile = ParadiseLostBlocks.GRAVITITE_ORE.getDefaultState();
        }
    }
    ...
    public boolean isFastFloater() {
        return this.floatTile.isIn(ParadiseLostBlockTags.FAST_FLOATERS) && !this.partOfStructure;
    }
