...
public class AetherColorProviders {
    ...
    private static void initBlocks() {
        ...
        ColorProviderRegistryImpl.BLOCK.register(((state, world, pos, tintIndex) -> world != null && pos != null ? BiomeColors.getGrassColor(world, pos) : GrassColors.getColor(0.5D, 1.0D)), AetherBlocks.AETHER_GRASS_BLOCK, AetherBlocks.AETHER_GRASS, AetherBlocks.AETHER_GRASS_FLOWERING, AetherBlocks.AETHER_TALL_GRASS, AetherBlocks.AETHER_FERN, AetherBlocks.AETHER_BUSH);
        ...
        // ideally we shouldn't go through the entire block registry, but it only runs once anyway.
        Registry.BLOCK.stream()
                .filter(block -> block instanceof DynamicColorBlock)
                .forEach(block -> ColorProviderRegistryImpl.BLOCK.register(((DynamicColorBlock) block).getBlockColorProvider(), block));
    }
    ...
    private static void initItems() {
        ...
        Registry.ITEM.stream()
                .filter(item -> item instanceof BlockItem blockItem && blockItem.getBlock() instanceof DynamicColorBlock)
                .forEach(item -> ColorProviderRegistryImpl.ITEM.register(((DynamicColorBlock) ((BlockItem) item).getBlock()).getBlockItemColorProvider(), item));
    }
}
